...
# --- 週次トレンドの描画 (ADDED 専用) ---
def plot_weekly_added_trends(project_info, lr_data, sr_data, major_releases):
    print(f"\n--- Plotting Combined Weekly SATD ADDED Trends for {project_info['project_name']} ---")
    ...
    # ADDEDのみのプロット設定
    config = {'metric': 'SATD_ADDED', 'norm_metric': 'Normalized_ADDED', 'title': 'ADDED',
              'ylabel_raw': 'Weekly Raw Count', 'ylabel_norm': 'Count per 1000 Commits',
              'label_raw': 'Weekly ADDED', 'label_norm': 'Normalized ADDED'}
    ...
    # 添付画像(Equinox)のようにスパイクがある場合、y軸の上限をクリップする (例: 3500)
    # current_max_y_norm = min(current_max_y_norm, 3500)
...
def main():
    db_conn = None
    added_summary, added_norm_summary = {}, {}
    commit_summary = {}
        ...
        if db_conn.is_connected():
            ...
            for project in STUDY_PROJECTS:
                ...
                if lr_data and sr_data:
                    try:
                        plot_weekly_added_trends(project, lr_data, sr_data,
                                                 major_releases_for_project if major_releases_for_project else [])
                    except Exception as plot_weekly_err:
                        print(f"Error plotting weekly ADDED trends for {project['project_name']}: {plot_weekly_err}")
                else:
